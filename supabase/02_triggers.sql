...
-- ============================================================
-- 函数8：更新用户显示名称（前端调用，绕过 RLS）
-- 同时同步更新该用户创建的所有交易记录的 creator_name
-- ============================================================
...
BEGIN
  -- 更新用户名
  UPDATE public.app_users SET name = p_new_name WHERE id = p_user_id;
  -- 同步更新该用户创建的所有交易记录
  UPDATE public.transactions SET creator_name = p_new_name WHERE creator_id = p_user_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
RETURNS VOID AS $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.transactions WHERE account_id = p_account_id LIMIT 1) THEN
    RAISE EXCEPTION '该账户下还有交易记录，无法删除';
  END IF;
